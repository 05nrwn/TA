document.addEventListener('DOMContentLoaded', function () {
    var ctx = document.getElementById('furtherStudyCostChart').getContext('2d');
    var furtherStudyCostChart = null;

    function updateChart(data) {
        var labels = data.map(function (item) {
            return item.sumber_biaya_studi_lanjut;
        });
        var counts = data.map(function (item) {
            return item.count;
        });

        if (furtherStudyCostChart) {
            furtherStudyCostChart.destroy();
        }

        furtherStudyCostChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: labels,
                datasets: [{
    ...
    function fetchData(dataType, tahunLulus, studyProgram) {
        fetch(`/filter-furtherstudycost?data_type=${dataType}&tahun_lulus=${tahunLulus}&program_studi=${studyProgram}`)
    ...
    function fetchData(tahunLulus, studyProgram) {
        fetch(`/filter-furtherstudycost?tahun_lulus=${tahunLulus}&program_studi=${studyProgram}`)
            .then(response => response.json())
            .then(data => {
                console.log('Fetched data:', data); // Debugging: Check the fetched data
                updateChart(data['further_study_cost']);
            });
    }
