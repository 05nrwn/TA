document.addEventListener('DOMContentLoaded', function () {
    var ctx = document.getElementById('workGradeAppropriateChart').getContext('2d');
    var workGradeAppropriateChart = null;

    function updateChart(data) {
        var labels = data.map(function (item) {
            return item.tingkat_pendidikan_sesuai_pekerjaan;
        });
        var counts = data.map(function (item) {
            return item.count;
        });

        if (workGradeAppropriateChart) {
            workGradeAppropriateChart.destroy();
        }

        workGradeAppropriateChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: labels,
                datasets: [{
    ...
    function fetchData(tahunLulus, studyProgram) {
        fetch(`/filter-workgradeappropriate?tahun_lulus=${tahunLulus}&program_studi=${studyProgram}`)
            .then(response => response.json())
            .then(data => {
                console.log('Fetched data:', data); // Debugging: Check the fetched data
                updateChart(data['work_grade_appropriate']);
            });
    }
