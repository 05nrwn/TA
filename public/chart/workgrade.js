document.addEventListener('DOMContentLoaded', function () {
    var ctx = document.getElementById('workGradeChart').getContext('2d');
    var workGradeChart = null;

    function updateChart(data) {
        var labels = data.map(function (item) {
            return item.tingkatan_tempat_kerja;
        });
        ...
        if (workGradeChart) {
            workGradeChart.destroy();
        }

        workGradeChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: labels,
                datasets: [{
    ...
    function fetchData(dataType, tahunLulus, studyProgram) {
        fetch(`/filter-workgrade?data_type=${dataType}&tahun_lulus=${tahunLulus}&program_studi=${studyProgram}`)
    ...
    function fetchData(tahunLulus, studyProgram) {
        fetch(`/filter-workgrade?tahun_lulus=${tahunLulus}&program_studi=${studyProgram}`)
            .then(response => response.json())
            .then(data => {
                console.log('Fetched data:', data); // Debugging: Check the fetched data
                updateChart(data['work_grade']);
            });
    }
