document.addEventListener('DOMContentLoaded', function () {
    var ctx = document.getElementById('workingPlaceRegencyChart').getContext('2d');
    var workingPlaceRegencyChart = null;

    function updateChart(data) {
        var labels = data.map(function (item) {
            return item.lokasi_kab_kota_bekerja;
        });
        var counts = data.map(function (item) {
            return item.count;
        });

        if (workingPlaceRegencyChart) {
            workingPlaceRegencyChart.destroy();
        }

        workingPlaceRegencyChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: labels,
                datasets: [{
    ...
    function fetchData(dataType, tahunLulus, studyProgram) {
        fetch(`/filter-regency?data_type=${dataType}&tahun_lulus=${tahunLulus}&program_studi=${studyProgram}`)
    ...
    function fetchData(tahunLulus, studyProgram) {
        fetch(`/filter-regency?tahun_lulus=${tahunLulus}&program_studi=${studyProgram}`)
            .then(response => response.json())
            .then(data => {
                console.log('Fetched data:', data); // Debugging: Check the fetched data
                updateChart(data['working_place_regency']);
            });
    }
