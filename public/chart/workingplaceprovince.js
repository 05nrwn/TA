document.addEventListener('DOMContentLoaded', function () {
    var ctx = document.getElementById('workingPlaceProvinceChart').getContext('2d');
    var workingPlaceProvinceChart = null;

    function updateChart(data) {
        var labels = data.map(function (item) {
            return item.lokasi_provinsi_bekerja;
        });
        var counts = data.map(function (item) {
            return item.count;
        });

        if (workingPlaceProvinceChart) {
            workingPlaceProvinceChart.destroy();
        }

        workingPlaceProvinceChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: labels,
                datasets: [{
    ...
    function fetchData(tahunLulus, studyProgram) {
        fetch(`/filter-province?tahun_lulus=${tahunLulus}&program_studi=${studyProgram}`)
            .then(response => response.json())
            .then(data => {
                console.log('Fetched data:', data); // Debugging: Check the fetched data
                updateChart(data['working_place_province']);
            });
    }
